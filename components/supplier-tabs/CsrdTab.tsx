...
export function CsrdTab({ 
  ...
  const fetchExistingDocuments = async () => {
    try {
      console.log('Fetching existing CSRD documents for supplier:', supplier.id)
  ...
  const handleModalUpload = async () => {
    if (selectedDocumentIndex !== null && selectedFile && user) {
      try {
        ...
        const filePath = `csrd-docs/${user.id}/${fileName}` // Use category and user ID for better organization
